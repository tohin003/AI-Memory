...
async function init() {
    try {
        ...
        contents = document.querySelectorAll('.view-section');
        ...
        if (userProfile && profilePopover) {
            // Set User Info
            popoverUsername.textContent = username || 'User';
            popoverEmail.textContent = email || 'No Email';

            // Try to set profile image if available (e.g. from Google Auth if we had it)
            // For now, we use a generated avatar or default
            // profileImg.src = ... 
            ...
            document.addEventListener('click', (e) => {
                if (!profilePopover.contains(e.target) && !userProfile.contains(e.target)) {
        ...
        if (logoutBtn) {
            logoutBtn.addEventListener('click', async () => {
                if (confirm('Are you sure you want to logout?')) {
                    await chrome.storage.local.remove(['authToken', 'username', 'email']);
                    window.location.href = '../auth/login.html';
                }
            });
        }
